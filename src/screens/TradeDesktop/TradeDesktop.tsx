...
export const TradeDesktop = (): JSX.Element => {
  ...
  const handlePlaceOrder = () => {
    ...
    const newPosition = {
      ...
      team: "MANC",
      type: tradeType,
      profit: tradeType === "long" ? `+$${(invest * 0.3).toFixed(2)}` : `-$${(invest * 0.3).toFixed(2)}`,
    ...
    toast.success(`${tradeType === 'long' ? 'Long' : 'Short'} order for ${isUSDC ? '$' + invest.toFixed(2) : shares + ' shares'} placed successfully!`);
  };
